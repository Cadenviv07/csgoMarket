...
fn expected_price(baseline: f64, crashed_price: f64, hours_elapsed: f64, tau: f64) -> f64{
    let e_val: f64 = std::f64::consts::E;
    let expected_price: f64 = baseline - (baseline - crashed_price) * (e_val.exp(-1*hours_elapsed/tau));
}
